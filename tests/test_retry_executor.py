...
from src.mcp_code_indexer.database.retry_executor import (
    RetryExecutor, RetryConfig, RetryStats, DatabaseLockError, create_retry_executor
)
from src.mcp_code_indexer.database.exceptions import (
    DatabaseError, DatabaseBusyError, classify_sqlite_error, is_retryable_error
)
    ...
    async def test_concurrent_operations(self, retry_executor):
        ...
        tasks = [
            retry_executor.execute_with_retry(
                lambda oid=i: concurrent_operation(oid),
                f"concurrent_op_{i}"
            )
            for i in range(10)
        ]
...
class TestRetryExecutorWithRealDatabase:
    ...
    async def test_database_operations_with_retries(self, temp_database):
        ...
        async with aiosqlite.connect(temp_database) as db:
            cursor = await db.execute("SELECT data FROM test_table WHERE data = ?", ("test_data",))
    ...
    async def test_simulated_lock_contention(self, temp_database):
        ...
        async def contended_operation():
            ...
            async with aiosqlite.connect(temp_database) as db:
                await db.execute("INSERT INTO test_table (data) VALUES (?)", ("contended",))
                await db.commit()
