...
from mcp_code_indexer.vector_mode.const import VoyageModel
...
class TestTurbopufferClientValidation:
    ...
    def test_validate_api_access_401_unauthorized(self, mock_turbopuffer_client):
        ...
        # Mock 401 error
        mock_turbopuffer_client.client.namespaces.side_effect = Exception(
            "401 Unauthorized"
        )
    ...
    def test_validate_api_access_generic_error(self, mock_turbopuffer_client):
        ...
        with pytest.raises(
            RuntimeError, match="Turbopuffer API access validation failed"
        ):
            mock_turbopuffer_client.validate_api_access()
    ...
    def test_delete_vectors_for_file_success(self, mock_turbopuffer_client):
        ...
        # Test deletion
        result = mock_turbopuffer_client.delete_vectors_for_file(
            "test_namespace", "/test/file.py"
        )
    ...
    def test_delete_vectors_for_file_query_failure(self, mock_turbopuffer_client):
        ...
        with pytest.raises(
            RuntimeError, match="File vector deletion failed: Query failed"
        ):
            mock_turbopuffer_client.delete_vectors_for_file(
                "test_namespace", "/test/file.py"
            )
    ...
    def test_delete_vectors_for_file_delete_failure(self, mock_turbopuffer_client):
        ...
        with pytest.raises(
            RuntimeError,
            match="File vector deletion failed: Vector deletion failed: Delete failed",
        ):
            mock_turbopuffer_client.delete_vectors_for_file(
                "test_namespace", "/test/file.py"
            )
...
class TestVoyageClientValidation:
    ...
    def mock_voyage_client(self) -> VoyageClient:
        ...
        with patch("voyageai.Client") as mock_sdk:
            ...
            client = VoyageClient(
                api_key="test-api-key", model=VoyageModel.VOYAGE_CODE_2
            )
            return client
    ...
    def test_validate_api_access_rate_limit(self, mock_voyage_client):
        ...
        # Mock rate limit error
        mock_voyage_client.client.embed.side_effect = Exception(
            "429 Rate limit exceeded"
        )
    ...
    def test_validate_api_access_generic_error(self, mock_voyage_client):
        ...
        with pytest.raises(
            RuntimeError, match="Voyage AI API access validation failed"
        ):
            mock_voyage_client.validate_api_access()
