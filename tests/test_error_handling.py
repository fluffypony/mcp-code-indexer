...
class TestMCPErrors:
    ...
    def test_database_error(self):
        ...
        error = DatabaseError(
            "Database connection failed", details={"host": "localhost"}
        )
...
class TestToolMiddleware:
    ...
    async def test_validate_tool_arguments(self):
        ...
        with pytest.raises(ValidationError, match="Unexpected fields"):
            await test_handler(
                {"required1": "value1", "required2": "value2", "unexpected": "value"}
            )
...
class TestLoggingConfiguration:
    ...
    def test_setup_logging_basic(self, tmp_path):
        ...
        logger = setup_logging(
            log_level="DEBUG", log_file=log_file, enable_file_logging=True
        )
